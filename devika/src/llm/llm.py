...
class LLM:
    ...
    def inference(self, prompt: str, project_name: str) -> str:
        ...
        try:
            ...
            print(f"DEBUG → Starting inference with model: {model} and model_name: {model_name}")
                ...
                try:
                    while True:
                        ...
                        if int(elapsed_time) % 5 == 0:
                            print(f"DEBUG → Inference in progress: {elapsed_seconds}s")
                        ...
                        if elapsed_time > self.timeout_inference:
                            print("DEBUG → Inference timed out!")
                            raise concurrent.futures.TimeoutError
                        if future.done():
                            print("DEBUG → Inference future completed")
                            break
                        time.sleep(0.5)

                    response = future.result(timeout=self.timeout_inference).strip()
                    print(f"DEBUG → Inference successful, response length: {len(response)}")
